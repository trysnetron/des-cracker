...
#define DRIVER_NAME "DEScracker"
#define COMPATIBLE_STRING "xlnx,des-cracker-1.0"

MODULE_AUTHOR("Yohann Jacob Sandvik and Trym Sneltvedt");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION(DRIVER_NAME ": software driver for DES cracker");
 ...
 * difference with the classic init function is that we have access to
...
static int dht11_probe(struct platform_device *pdev) {
	...
	// register cdev structure to kernel, with device major number dev
	...
	// create device class
	if (IS_ERR(cl = class_create(THIS_MODULE, "dht11"))) {
		dev_err(&pdev->dev, "could not create device class\n");
		goto err_delete_cdev;
	}

	// populate device info under class
	if(IS_ERR(dev_ret = device_create(cl, NULL, dev, NULL, "dht11"))) {
